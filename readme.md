...
```
tanzu apps workload create spring-petclinic-kind -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/spring-petclinic-kind-source \
  --type web
```
...
```
tanzu apps workload get spring-petclinic-kind
```
